...
def filter_data(df: pandas.DataFrame) -> pandas.DataFrame:
    columns_to_drop = [
        'id',
        'url',
        'region',
...
def remove_outliers(df: pandas.DataFrame) -> pandas.DataFrame:
...
def create_features(df: pandas.DataFrame) -> pandas.DataFrame:
    def short_model(x):
...
def pipeline() -> None:
    df = pandas.read_csv(f'{path}/data/train/homework.csv')
